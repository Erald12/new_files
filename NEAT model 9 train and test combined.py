...
def run_neat(config_path):
    ...
    winner = p.run(evaluate_genome, 10)
    print('\nBest genome:\n{!s}'.format(winner))
...
def test_model(genome, test_data, config_path):
    ...
    if len(set) == 0:
        ...
        winrate = set.count(1) / len(set)
    print(f'Winrate: {round(winrate, 2) * 100}%, Wins: {win}, Loss: {loss}, Balance: {total_profit}')
    return action_list
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            actions = test_model(best, data_inputs, config_path)
