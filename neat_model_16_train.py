...
while True:
        ...
        def evaluate_genomes(genomes, config, prune_every=10):
            ...
            for genome_id, genome in genomes:
                ...
                while True:
                    ...
                    state = np.concatenate([[signal()], env.data.iloc[env.current_step, :15].values])
                    action = np.argmax(net.activate(state)) - 1  # Map to -1, 0, 1

                    # Execute the action in the environment
                    balance, done, winloss, total_profits, action_list, fit = env.step(action)
                    if done:
                        break
                ...
                while True:
                    ...
                    # Ensure state is compatible with neural network input size
                    state2 = np.concatenate([[signal2()], env2.data.iloc[env2.current_step, :15].values])
                    action2 = np.argmax(net.activate(state2)) - 1  # Map to -1, 0, 1

                    # Execute the action in the environment
                    balance2, done2, winloss2, total_profits2, action_list2, fit2 = env2.step(action2)
                    if done2:
                        break
    ...
    def test_model(net, test_data, config_path):
        ...
        while True:
            ...
            # Execute the action in the environment
            balance, done, winloss, total_profits2, action_list, fit = env2.step(action2)
            if done:
                break
