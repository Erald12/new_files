...
import time
import pickle
from neat import DefaultStagnation, Population
...
def evaluate_genome(genomes, config):
    for genome_id, genome in genomes:
        ...
        try:
            ...
            env = TradingEnvironment(second_half)
            env.reset()
...
def rerun(config_path):
    # Define a new population using the config
    config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction,
                         neat.DefaultSpeciesSet, neat.DefaultStagnation,
                         config_path)

    # Custom Reproduction class, starting with the best genome
    class CustomReproduction(neat.DefaultReproduction):
        def __init__(self, reproduction_config, genome_type, stagnation):
            super().__init__(reproduction_config, genome_type, stagnation)

        def create_new(self, num_offspring, species, generation):
            if generation == 0:
                # First generation uses the best genome only
                for _ in range(num_offspring):
                    genome = self.create_genome()
                    genome.configure_crossover(best_genome, best_genome, config.genome_config)
                    yield genome
            else:
                return super().create_new(num_offspring, species, generation)

    # Custom Population class that uses CustomReproduction and ensures proper stagnation
    class CustomPopulation(Population):
        def __init__(self, config):
            self.reproduction = CustomReproduction(config.reproduction_config, config.genome_type, config.genome_config)
            self.species_set = DefaultSpeciesSet(config.species_set_config, reporters=[])
            self.stagnation = DefaultStagnation(config.stagnation_config, reporters=[])

            # Initialize the base class with customized components
            super().__init__(config)

    # Initialize the custom population
    pop = CustomPopulation(config)

    # Add reporters for console output and statistics
    pop.add_reporter(neat.StdOutReporter(True))
    stats = neat.StatisticsReporter()
    pop.add_reporter(stats)

    # Resume training with the best genome as the starting point
    winner = pop.run(evaluate_genome, 100)  # Run for an additional 50 generations or as needed
    print('\nBest genome:\n{!s}'.format(winner))
...
if __name__ == "__main__":
    ...
    best_genome = load_best_genome('best_genome2.pkl')
